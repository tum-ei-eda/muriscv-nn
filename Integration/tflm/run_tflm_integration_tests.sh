...
if [ "${SKIP_BUILD}" == OFF ]; then
  ...
  mkdir build
  cd build
  cmake -DRISCV_GCC_PREFIX=$(pwd)/../Toolchain/${RV_ARCH}/ -DENABLE_INTG_TESTS=ON -DTOOLCHAIN=${TOOLCHAIN} -DUSE_VEXT=${USE_VEXT} -DUSE_PEXT=${USE_PEXT} -DUSE_CMSIS_NN_WRAPPER=ON ..
  make all
