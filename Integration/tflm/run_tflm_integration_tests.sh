...
if [ "${SKIP_BUILD}" == OFF ]; then
  ...
  then
    ...
    cmake -DRISCV_GCC_PREFIX=$(pwd)/../Toolchain/${RV_ARCH}/ -DRISCV_LLVM_PREFIX=$(pwd)/../Toolchain/llvm/bin/ -DENABLE_INTG_TESTS=ON -DTOOLCHAIN=${TOOLCHAIN} -DUSE_PORTABLE=${USE_PORTABLE} -DUSE_VEXT=${USE_VEXT} -DUSE_PEXT=${USE_PEXT} -DDISABLE_TVM_INTG_TESTS=ON -DENABLE_UNIT_TESTS=OFF ..
  fi
  make all -j`nproc`
