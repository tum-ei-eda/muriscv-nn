...
logging.basicConfig(level=logging.INFO, format='### PYTHON ### [%(asctime)s] %(pathname)s:%(lineno)d %(levelname)s - %(message)s')
...
def setup_etiss(config: Config):
    ...
    try:
        git.Repo.clone_from(config.ETISS_GIT_URL, config.etiss_dir,
                            branch=config.ETISS_GIT_BRANCH, progress=Progress())
    except:
    ...
    subprocess.run(['cmake',
                   '-S', config.etiss_dir_sw,
                   '-B', config.etiss_dir_sw_build,
    ...
    try:
        ...
        output = subprocess.run(run_args, check=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT).stdout.decode('utf-8')
...
def main():
    ...
    config = Config(muriscv_nn_dir=os.path.abspath(os.path.join(__file__, os.path.pardir, os.path.pardir, os.path.pardir)))
