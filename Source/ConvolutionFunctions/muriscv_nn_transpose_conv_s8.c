                                          ...
                                          int8_t *output_data)
{
    ...
    const int32_t col_buf_size = muriscv_nn_transpose_conv_s8_get_buffer_size(input_dims, filter_dims, output_dims);
    ...
    while (batch_cnt)
    {
        if (bias_data == NULL)
        {
            muriscv_nn_memset_s8((int8_t *)img_buf_ptr, 0, output_x * output_y * output_ch * sizeof(int32_t));
        }
        else
        {
        ...
        muriscv_nn_memset_s8((int8_t *)col_data_ptr, 0, col_buf_size);

        for (int i_output_ch = 0; i_output_ch < output_ch; i_output_ch++)
        {
            muriscv_nn_mat_mult_nt_t_s8_s32(input_data_ptr,
                                        filter_data_ptr,
                                        col_data_ptr,
                                        input_size,
                                        input_ch,
        ...
        for (int i = 0; i < output_x * output_y; i++)
        {
            ...
            {
                int32_t result =
                    muriscv_nn_requantize(img_data[i_output_ch], output_multiplier[i_output_ch], output_shift[i_output_ch]);
                result += out_offset;
