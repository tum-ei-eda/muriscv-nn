                                                  ...
                                                  int8_t *output_data)
{
    ...
    if (reverse_conv_possible && reverse_conv_efficient)
    {
        ...
        const muriscv_nn_dims transposed_input_dims = {input_n, input_h * stride_h, input_w * stride_w, input_c};
        const muriscv_nn_dims upscale_dims = {0, stride_h, stride_w, 0};
        ...
        for (int32_t i = 0; i < output_c; i++)
        {
            for (int32_t y = 0; y < filter_h; y++)
            {
                ...
                {
                    out_ptr -= input_c;
                    muriscv_nn_memcpy_s8(out_ptr, in_ptr, input_c * sizeof(int8_t));
                    in_ptr += input_c;
                }
            }
