                               ...
                               const muriscv_nn_dims *post_pad)
{
    ...
    muriscv_nn_memset_s8(output, pad_value, batch_block_size * pre_pad->n);
    output += batch_block_size * pre_pad->n;
    for (int32_t b = 0; b < input_size->n; b++)
    {

        muriscv_nn_memset_s8(output, pad_value, row_block_size * pre_pad->h);
        output += row_block_size * pre_pad->h;
        for (int32_t y = 0; y < input_size->h; y++)
        {

            muriscv_nn_memset_s8(output, pad_value, col_block_size * pre_pad->w);
            output += col_block_size * pre_pad->w;
            if (input_size->c == output_size.c)
            {
                muriscv_nn_memcpy_s8(output, input, input_size->w * input_size->c);
            ...
            else
            {
                for (int32_t x = 0; x < input_size->w; x++)
                {
                    ...
                    muriscv_nn_memset_s8(output, pad_value, pre_pad->c);
                    output += pre_pad->c;

                    muriscv_nn_memcpy_s8(output, input, input_size->c);
                    output += input_size->c;
                    input += input_size->c;

                    muriscv_nn_memset_s8(output, pad_value, post_pad->c);
                    output += post_pad->c;
                }
            }

            muriscv_nn_memset_s8(output, pad_value, col_block_size * post_pad->w);
            output += col_block_size * post_pad->w;
        }

        muriscv_nn_memset_s8(output, pad_value, row_block_size * post_pad->h);
        output += row_block_size * post_pad->h;
    }
    muriscv_nn_memset_s8(output, pad_value, batch_block_size * post_pad->n);
