...
void depthwise_int4_1_muriscv_nn_depthwise_conv_s4_opt(void)
{
    ...
    ctx.size = muriscv_nn_depthwise_conv_s4_opt_get_buffer_size(&input_dims, &filter_dims);

    TEST_ASSERT_TRUE(ctx.size > 0);

    ctx.buf = malloc(ctx.size);

    muriscv_nn_status result = muriscv_nn_depthwise_conv_s4_opt(&ctx,
                                                           &dw_conv_params,
                                                           &quant_params,
                                                           &input_dims,
                                                           input_data,
    ...
    ctx.size = 0;
    ctx.buf = malloc(ctx.size);
    result = muriscv_nn_depthwise_conv_s4(&ctx,
                                   &dw_conv_params,
                                   &quant_params,
                                   &input_dims,
                                   input_data,
...
void depthwise_int4_2_muriscv_nn_depthwise_conv_s4_opt(void)
{
    const muriscv_nn_status expected = MURISCV_NN_SUCCESS;
...
void depthwise_int4_3_muriscv_nn_depthwise_conv_s4_opt(void)
{
    const muriscv_nn_status expected = MURISCV_NN_SUCCESS;
...
void depthwise_int4_4_muriscv_nn_depthwise_conv_s4_opt(void)
{
    const muriscv_nn_status expected = MURISCV_NN_SUCCESS;
