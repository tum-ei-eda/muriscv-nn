// Modifications copyright (C) 2024 Chair of Electronic Design Automation, TUM
// Generated by test_settings.py using tensorflow version 2.14.0 (Keras version 2.14.0).
// Interpreter from tflite_runtime version 2.16.0 and revision 0.6.0-154906-gb2493fdf794.
#pragma once
#include <stdint.h>

const int32_t depthwise_int4_generic_2_output_mult[9] =
    {1442056031, 1442056031, 1442056031, 1442056031, 1442056031, 1442056031, 1442056031, 1442056031, 1442056031};
